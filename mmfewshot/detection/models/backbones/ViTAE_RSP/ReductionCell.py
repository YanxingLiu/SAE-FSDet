...
class PRM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.share_weights:
            # 多个共享权重扩张率不同的扩张卷积
            padding = math.ceil((
                (self.kernel_size - 1) * self.dilations[0] + 1 - self.stride) /
                                2)
            ...
            for i in range(1, len(self.dilations)):
                padding = math.ceil(
                    ((self.kernel_size - 1) * self.dilations[i] + 1 -
                     self.stride) / 2)
        ...
        if self.op == 'sum':
            ...
            # 级联合并
            y = y.permute(0, 4, 1, 2,
                          3).flatten(3).reshape(B, N * C,
                                                W * H).permute(0, 2,
                                                               1).contiguous()
        else:
            raise NotImplementedError(
                f'no such operation: {self.op} for multi-levels!')
        return y, (W, H)
...
class ReductionCell(nn.Module):
    ...
    def __init__(self,
        ...
        if tokens_type == 'pooling':
            ...
            self.pool = nn.MaxPool2d(
                downsample_ratios, stride=downsample_ratios, padding=0)
        ...
        if tokens_type == 'performer':
            ...
            self.attn = SwinTransformerBlock(
                in_dim=in_chans,
                out_dim=token_dims,
                input_resolution=(self.img_size // self.downsample_ratios,
        ...
        if gamma:
            self.gamma2 = nn.Parameter(
                init_values * torch.ones(token_dims), requires_grad=True)
        ...
        self.num_patches = (img_size // 2) * (
            img_size // 2
        )  # there are 3 sfot split, stride are 4,2,2 separately
    ...
    def forward(self, x, H, W):
        ...
        if self.tokens_type == 'swin':
            ...
            )  # nW*B, window_size*window_size, C， 进行窗口attention
            attn_windows = attn_windows.view(-1, self.window_size,
                                             self.window_size,
                                             self.token_dims)  # 窗口级特征1D变2D
            shifted_x = window_reverse(attn_windows, self.window_size, Hp,
                                       Wp)  # B H' W' C， 将窗口特征恢复成普通特征
            x = shifted_x
            ...
            x = x + self.attn.drop_path(
                self.gamma3 * self.attn.mlp(self.attn.norm2(x)))
        else:
