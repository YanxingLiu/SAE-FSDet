...
from mmfewshot.detection.models.utils.checkpoint_vitae import load_checkpoint
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, inter_channel=32, out_channels=48, img_size=None):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                ...
                stride=2,
                padding=1,
                bias=False), nn.BatchNorm2d(inter_channel),
    ...
    def flops(self, ) -> float:
        flops = 0
        flops += 3 * self.inter_channel * self.img_size[0] * self.img_size[
            1] // 4 * 9
        flops += self.img_size[0] * self.img_size[1] // 4 * self.inter_channel
        flops += self.inter_channel * self.out_channel * self.img_size[
            0] * self.img_size[1] // 16 * 9
        flops += self.img_size[0] * self.img_size[1] // 16 * self.out_channel
        flops += self.out_channel * self.out_channel * self.img_size[
            0] * self.img_size[1] // 16
        return flops
    ...
    def __init__(self,
        ...
        self.NC = nn.ModuleList([
            NormalCell(
                ...
                window_size=window_size,
                shift_size=0,
                relative_pos=relative_pos) for i in range(NC_depth)
        ])
...
class ViTAE_RSP(nn.Module):
    ...
    def __init__(self,
                 ...
                 RC_op='cat',
                 RC_tokens_type=[
                     'performer', 'transformer', 'transformer', 'transformer'
                 ],
                 NC_tokens_type='transformer',
                 RC_group=[1, 1, 1, 1],
                 NC_group=[1, 32, 64, 64],
        ...
        self.stages = stages
        repeatOrNot = (lambda x, y, z=list: x
                       if isinstance(x, z) else [x for _ in range(y)])
        self.embed_dims = repeatOrNot(embed_dims, stages)
        self.tokens_dims = token_dims if isinstance(token_dims, list) else [
            token_dims * (2**i) for i in range(stages)
        ]
