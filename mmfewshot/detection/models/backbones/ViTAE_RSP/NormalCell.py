...
class AttentionPerformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.head_cnt):
            self.w[i] = nn.Parameter(
                nn.init.orthogonal_(self.w[i]) * math.sqrt(self.m),
                requires_grad=False)
    ...
    def attn(self, x):
        B, N, C = x.shape
        kqv = self.kqv(x).reshape(B, N, 3, self.head_cnt,
        ...
        y = torch.einsum('bhti,bhni->bhtn', qp, kptv) / (D.repeat(
            1, 1, 1, self.head_dim) + self.epsilon)  # (B, H, T, emb)/Diag
    ...
    def forward(self, x, H, W):
        ...
        if self.class_token:
            ...
            convX = self.drop_path(
                self.gamma2 * self.PCM(shortcut[:, 1:, :].view(
                    b, wh, wh, c).permute(0, 3, 1, 2).contiguous()).permute(
            ...
            #wh = int(math.sqrt(n))
            # shortcut过PCM再变成1D特征
            convX = self.drop_path(self.gamma2 * self.PCM(
                shortcut.view(b, H, W,
                              c).permute(0, 3, 1, 2).contiguous()).permute(
                                  0, 2, 3, 1).contiguous().view(b, n, c))
            # 三路合并
