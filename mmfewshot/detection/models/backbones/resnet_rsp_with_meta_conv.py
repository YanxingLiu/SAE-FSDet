...
class ResNetRSPWithMetaConv(ResNetRSP):
    """ResNetRSP with `meta_conv` to handle different inputs in metarcnn and
    fsdetview.

    When input with shape (N, 3, H, W) from images, the network will use
    `conv1` as regular ResNetRSP. When input with shape (N, 4, H, W) from (image +
    ...
    def forward(self, x: Tensor, use_meta_conv: bool = False) -> Tuple[Tensor]:
        ...
        When input with shape (N, 3, H, W) from images, the network will use
        `conv1` as regular ResNetRSP. When input with shape (N, 4, H, W) from
