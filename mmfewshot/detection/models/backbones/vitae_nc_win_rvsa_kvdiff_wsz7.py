...
from ..utils.checkpoint_vitae import load_checkpoint
...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.rel_pos_h = nn.Parameter(torch.zeros(rel_sp_dim,
                                                  head_dim))  # 2ws-1,C'
        self.rel_pos_w = nn.Parameter(torch.zeros(rel_sp_dim,
                                                  head_dim))  # 2ws-1,C'
...
def calc_rel_pos_spatial(
    ...
    r_q = q[:, :, sp_idx:].reshape(B, n_head, q_h, q_w,
                                   dim)  # B, H, qwh, qww, C
...
class RotatedVariedSizeWindowAttention_KVDIFF(nn.Module):
    ...
    def __init__(self,
        ...
        if self.learnable:
            ...
            self.sampling_offsets_k = nn.Sequential(
                ...
                nn.Conv2d(
                    dim,
                    self.num_heads * self.num_deform * 2,
                    kernel_size=1,
                    stride=1))
            ...
            self.sampling_angles_k = nn.Sequential(
                ...
                nn.Conv2d(
                    dim,
                    self.num_heads * self.num_deform * 1,
                    kernel_size=1,
                    stride=1))
        ...
        if self.relative_pos_embedding:
            ...
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros((window_size + attn_window_size - 1) *
            ...
            relative_coords[:, :, 0] += self.attn_ws - \
    ...
    def forward(self, x, H, W):
        ...
        image_reference = torch.stack(
            torch.meshgrid(image_reference_w, image_reference_h),
            0).permute(0, 2, 1).unsqueeze(0)  # 1, 2, H, W

        # position of the window relative to the image center
        window_reference = nn.functional.avg_pool2d(
            image_reference, kernel_size=self.ws)  # 1,2, nh, nw
        image_reference = image_reference.reshape(
            1, 2, window_num_h, self.ws, window_num_w,
            self.ws)  # 1, 2, nh, ws, nw, ws
        assert window_num_h == window_reference.shape[-2]
        assert window_num_w == window_reference.shape[-1]

        window_reference = window_reference.reshape(1, 2, window_num_h, 1,
                                                    window_num_w,
                                                    1)  # 1,2, nh,1, nw,1

        # coords of pixels in each window

        base_coords_h = torch.arange(self.attn_ws).to(
            x.device) * 2 * self.ws / self.attn_ws / (expand_h - 1)  # ws
        base_coords_h = (base_coords_h - base_coords_h.mean())
        base_coords_w = torch.arange(self.attn_ws).to(
            x.device) * 2 * self.ws / self.attn_ws / (
                expand_w - 1)
        ...
        window_coords = torch.stack(
            torch.meshgrid(expanded_base_coords_w, expanded_base_coords_h),
            0).permute(0, 2, 1).reshape(1, 2, window_num_h, self.attn_ws,
                                        window_num_w,
                                        self.attn_ws)  # 1, 2, nh, ws, nw, ws
        ...
        if self.restart_regression:
            ...
            coords_k = base_coords.repeat(b * self.num_heads, 1, 1, 1, 1,
                                          1)  # B*nH, 2, nh, ws, nw, ws
            coords_v = base_coords.repeat(b * self.num_heads, 1, 1, 1, 1,
                                          1)  # B*nH, 2, nh, ws, nw, ws
        if self.learnable:
            ...
            sampling_offsets_k = sampling_offsets_k.reshape(
                num_predict_total, 2, window_num_h, window_num_w)
            sampling_offsets_v = sampling_offsets_v.reshape(
                num_predict_total, 2, window_num_h, window_num_w)
            sampling_offsets_k[:, 0, ...] = sampling_offsets_k[:, 0, ...] / (
                h // self.ws)
            sampling_offsets_k[:, 1, ...] = sampling_offsets_k[:, 1, ...] / (
                w // self.ws)

            sampling_offsets_v[:, 0, ...] = sampling_offsets_v[:, 0, ...] / (
                h // self.ws)
            sampling_offsets_v[:, 1, ...] = sampling_offsets_v[:, 1, ...] / (
                w // self.ws)

            # kvdiff --- scale factors
            # B, heads*2, h // window_size, w // window_size
            sampling_scales_k = self.sampling_scales_k(x)
            sampling_scales_k = sampling_scales_k.reshape(
                num_predict_total, 2, window_num_h, window_num_w)

            # B, heads*2, h // window_size, w // window_size
            sampling_scales_v = self.sampling_scales_v(x)
            sampling_scales_v = sampling_scales_v.reshape(
                num_predict_total, 2, window_num_h, window_num_w)

            # kvdiff --- rotate factors

            sampling_angle_k = self.sampling_angles_k(x)
            sampling_angle_k = sampling_angle_k.reshape(
                num_predict_total, 1, window_num_h, window_num_w)

            sampling_angle_v = self.sampling_angles_v(x)
            sampling_angle_v = sampling_angle_v.reshape(
                num_predict_total, 1, window_num_h, window_num_w)

            # first scale

            window_coords_k = window_coords * \
                (sampling_scales_k[:, :, :, None, :, None] + 1)
            window_coords_v = window_coords * \
                (sampling_scales_v[:, :, :, None, :, None] + 1)
            ...
            window_coords_r_k[:,
                              0, :, :, :, :] = -window_coords_k[:, 1, :, :, :, :] * torch.sin(
                                  sampling_angle_k[:, 0, :, None, :, None]
                              ) + window_coords_k[:, 0, :, :, :, :] * torch.cos(
                                  sampling_angle_k[:, 0, :, None, :, None])
            window_coords_r_k[:,
                              1, :, :, :, :] = window_coords_k[:, 1, :, :, :, :] * torch.cos(
                                  sampling_angle_k[:, 0, :, None, :, None]
                              ) + window_coords_k[:, 0, :, :, :, :] * torch.sin(
                                  sampling_angle_k[:, 0, :, None, :, None])

            window_coords_r_v[:,
                              0, :, :, :, :] = -window_coords_v[:, 1, :, :, :, :] * torch.sin(
                                  sampling_angle_v[:, 0, :, None, :, None]
                              ) + window_coords_v[:, 0, :, :, :, :] * torch.cos(
                                  sampling_angle_v[:, 0, :, None, :, None])
            window_coords_r_v[:,
                              1, :, :, :, :] = window_coords_v[:, 1, :, :, :, :] * torch.cos(
                                  sampling_angle_v[:, 0, :, None, :, None]
                              ) + window_coords_v[:, 0, :, :, :, :] * torch.sin(
                                  sampling_angle_v[:, 0, :, None, :, None])

            # system transformation: window center -> image center

            coords_k = window_reference + window_coords_r_k + \
                sampling_offsets_k[:, :, :, None, :, None]
            coords_v = window_reference + window_coords_r_v + \
                sampling_offsets_v[:, :, :, None, :, None]

        # final offset
        sample_coords_k = coords_k.permute(0, 2, 3, 4, 5, 1).reshape(
            num_predict_total, self.attn_ws * window_num_h,
            self.attn_ws * window_num_w, 2)
        sample_coords_v = coords_v.permute(0, 2, 3, 4, 5, 1).reshape(
            num_predict_total, self.attn_ws * window_num_h,
            self.attn_ws * window_num_w, 2)
        ...
        qkv = self.qkv(shortcut.permute(0, 2, 3, 1).reshape(
            b, -1, self.dim)).permute(0, 2, 1).reshape(b, -1, h, w).reshape(
                b, 3, self.num_heads, self.out_dim // self.num_heads, h,
                w).transpose(1,
                             0).reshape(3 * b * self.num_heads,
                                        self.out_dim // self.num_heads, h, w)

        qkv = torch.nn.functional.pad(
            qkv,
            (padding_left, padding_right, padding_top, padding_down)).reshape(
                3, b * self.num_heads, self.out_dim // self.num_heads,
                h + padding_td, w + padding_lr)

        # b*self.num_heads, self.out_dim // self.num_heads, H，W
        q, k, v = qkv[0], qkv[1], qkv[2]

        k_selected = F.grid_sample(
            k.reshape(num_predict_total,
                      self.out_dim // self.num_heads // self.num_deform,
                      h + padding_td, w + padding_lr),
            grid=sample_coords_k,
            padding_mode='zeros',
            align_corners=True).reshape(b * self.num_heads,
                                        self.out_dim // self.num_heads,
                                        h + padding_td, w + padding_lr)
        v_selected = F.grid_sample(
            v.reshape(num_predict_total,
                      self.out_dim // self.num_heads // self.num_deform,
                      h + padding_td, w + padding_lr),
            grid=sample_coords_v,
            padding_mode='zeros',
            align_corners=True).reshape(b * self.num_heads,
                                        self.out_dim // self.num_heads,
                                        h + padding_td, w + padding_lr)

        q = q.reshape(b, self.num_heads, self.out_dim // self.num_heads,
                      window_num_h, self.ws, window_num_w,
                      self.ws).permute(0, 3, 5, 1, 4, 6, 2).reshape(
                          b * window_num_h * window_num_w, self.num_heads,
                          self.ws * self.ws, self.out_dim // self.num_heads)
        k = k_selected.reshape(b, self.num_heads,
                               self.out_dim // self.num_heads, window_num_h,
                               self.attn_ws,
                               window_num_w, self.attn_ws).permute(
                                   0, 3, 5, 1, 4, 6,
                                   2).reshape(b * window_num_h * window_num_w,
                                              self.num_heads,
                                              self.attn_ws * self.attn_ws,
                                              self.out_dim // self.num_heads)
        v = v_selected.reshape(b, self.num_heads,
                               self.out_dim // self.num_heads, window_num_h,
                               self.attn_ws,
                               window_num_w, self.attn_ws).permute(
                                   0, 3, 5, 1, 4, 6,
        ...
        dots = calc_rel_pos_spatial(dots, q, (self.ws, self.ws),
                                    (self.attn_ws, self.attn_ws),
        ...
        if self.relative_pos_embedding:
            relative_position_bias = self.relative_position_bias_table[
                self.relative_position_index.view(-1)].view(
                    self.ws * self.ws, self.attn_ws * self.attn_ws,
                    -1)  # Wh*Ww,Wh*Ww,nH
        ...
        out = out[:, :, padding_top:h + padding_top,
                  padding_left:w + padding_left]
    ...
    def flops(self, ):
        ...
        h, w = self.img_size[0] + self.shift_size + \
            self.padding_bottom, self.img_size[1] + \
            self.shift_size + self.padding_right
        ...
        if self.learnable:
            ...
            flops_sampling += 2 * (h/self.ws + w/self.ws) * \
                self.num_heads*2 * self.dim
...
class NormalCell(nn.Module):
    ...
    def __init__(self,
        ...
        mlp_hidden_dim = mlp_hidden_dim if mlp_hidden_dim is not None else int(
            dim * mlp_ratio)
    ...
    def forward(self, x, H, W):
        ...
        if self.class_token:
            ...
            convX = self.drop_path(
                self.PCM(x[:, 1:, :].view(b, wh, wh, c).permute(
                    0, 3, 1,
                    2).contiguous()).permute(0, 2, 3,
                                             1).contiguous().view(b, n, c))
