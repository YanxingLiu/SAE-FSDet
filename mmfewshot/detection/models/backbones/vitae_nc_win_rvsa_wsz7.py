...
class RotatedVariedSizeWindowAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.learnable:
            ...
            sampling_offsets[:, 0, ...] = sampling_offsets[:, 0, ...] / (
                h // self.ws)
            sampling_offsets[:, 1, ...] = sampling_offsets[:, 1, ...] / (
                w // self.ws)

            # scale factors
            # B, heads*2, h // window_size, w // window_size
            sampling_scales = self.sampling_scales(x)
            sampling_scales = sampling_scales.reshape(num_predict_total, 2,
                                                      window_num_h,
                                                      window_num_w)
            ...
            sampling_angle = sampling_angle.reshape(num_predict_total, 1,
                                                    window_num_h, window_num_w)
            ...
            window_coords_r[:,
                            0, :, :, :, :] = -window_coords[:, 1, :, :, :, :] * torch.sin(
                                sampling_angle[:, 0, :, None, :, None]
                            ) + window_coords[:, 0, :, :, :, :] * torch.cos(
                                sampling_angle[:, 0, :, None, :, None])
            window_coords_r[:,
                            1, :, :, :, :] = window_coords[:, 1, :, :, :, :] * torch.cos(
                                sampling_angle[:, 0, :, None, :, None]
                            ) + window_coords[:, 0, :, :, :, :] * torch.sin(
                                sampling_angle[:, 0, :, None, :, None])

            # system transformation: window center -> image center

            coords = window_reference + window_coords_r + \
                sampling_offsets[:, :, :, None, :, None]

        # final offset
        sample_coords = coords.permute(0, 2, 3, 4, 5,
                                       1).reshape(num_predict_total,
