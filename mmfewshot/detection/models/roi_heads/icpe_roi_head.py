...
class ICPERoIHead(StandardRoIHead):
    ...
    def __init__(self,
        ...
        if prototype_aggregation is not None:
            self.prototype_aggregation = build_prototype_aggregation(
                copy.deepcopy(prototype_aggregation))
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        if self.support_without_mean:
            support_feat_all = self.extract_support_feats_without_mean(
                support_feats)
        else:
        ...
        for img_id in range(batch_size):
            ...
            support_feat, support_gt_labels = self.prototype_aggregation(
                support_feat, support_gt_labels_origin)
        ...
        if self.bbox_head.with_meta_cls_loss:
            ...
            loss_bbox['loss_meta_cls'] = torch.stack(
                loss_bbox['loss_meta_cls']).sum() / batch_size
    ...
    def extract_support_feats_without_mean(
            self, feats: List[Tensor]) -> List[Tensor]:
        out = []
        if self.with_shared_head:
            for lvl in range(len(feats)):
                out.append(
                    self.shared_head.forward_support_without_mean(feats[lvl]))
        else:
            out = feats
        return out
    ...
    def simple_test_bboxes(
        ...
        for class_id in support_feats_dict.keys():
            ...
            if self.support_without_mean:
                support_feat = self.extract_support_feats_without_mean(
                    [support_feat])[0]
            else:
