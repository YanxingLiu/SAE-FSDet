...
class imTED(TwoStageDetector):
    ...
    #             Defaults to None.
    #     """
    #     # super(TwoStageetector, self).init_weights(pretrained)
    #     pretrained=self.backbone.pretrained
    #     super(imTED, self).init_weights()
    ...
    def get_roi_feat(self, x, vit_feat):
        ...
        if self.with_mfm:
            ...
            fea.append(vit_feat[:, 1:, :].transpose(1,
                                                    2).reshape(B, -1, H,
                                                               W).contiguous())
            x = tuple(fea)
        else:
            x = [
                vit_feat[:, 1:, :].transpose(1, 2).reshape(B, -1, H,
                                                           W).contiguous()
    ...
    def forward_train(self,
        ...
        if len(x) == 2:
            ...
            roi_losses = self.roi_head.forward_train(
                self.get_roi_feat(x, vit_feat), img_metas, proposal_list,
    ...
    def forward_dummy(self, img):
        ...
        roi_outs = self.roi_head.forward_dummy(
            self.get_roi_feat(x, vit_feat), proposals)
