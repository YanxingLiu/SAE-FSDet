...
def print_per_class_recall(recalls,
                           classes,
                           iou_thrs,
                           proposal_idx,
                           row_idxs=None,
    ...
    iou_thrs = np.array(iou_thrs)
    if row_idxs is None:
        row_idxs = np.arange(len(recalls))
    if col_idxs is None:
        col_idxs = np.arange(iou_thrs.size)
    row_header = ['iou thrs'] + iou_thrs[col_idxs].tolist()
    table_data = [row_header]
    for i, recall in enumerate(recalls):
        row = [f'{val:.3f}' for val in recall[proposal_idx].tolist()]
